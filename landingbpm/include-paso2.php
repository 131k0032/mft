...
//if($_COOKIE["txtdest2"]!=""){$VAR_ROUND = 2;}else{$VAR_ROUND=1;}
//if($_COOKIE["txtdest2"]==""){$VAR_TIPO = "ow";}else{$VAR_TIPO = "rt";}

$VAR_TIPO = $_COOKIE["txttipo"];
		...
		<div class="wrap">
            ...
            $sql = "select * from _precios where
               ((de = '".$VAR_DEL."' and a = '".$VAR_AL."')
                or (a = '".$VAR_DEL."' and de = '".$VAR_AL."') ) and hasta >= ".$VAR_PASAJEROS." and tipo = '".$VAR_TIPO."'";
            ...
            ?>
						<article class="result" STYLE="height:250px">
							...
							<div class="one-half heightfix">
								<h3><?php echo  $rstemp["vehiculo"]?> </h3>
								<ul>
									<li>
							...
							<div class="one-fourth heightfix">
								<div>
									...
									<a href="include-paso3.php" id="btnsel"  onclick="javascript:$('#sel').val('<?php echo $rstemp["vehiculo"]?>|<?php echo $rstemp["precio"]?>|<?php echo $VAR_ROUND ?>');
                                         var data = $('#sel').val();
                                         var arr = data.split('|');
